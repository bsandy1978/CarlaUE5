...
#     camera.listen(lambda image: image.save_to_disk("D:\CARLA_0.9.15\WindowsNoEditor\PythonAPI\examples\_out\data_out\Depth",color_converter=LogarithmicDepth))
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        #     raw_image = np.dstack([B,G,R]).astype(np.uint8) # BGR order
            ...
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            image.save_to_disk('_out/data_out/Depth-FPV/%08d' % image.frame)  #changes_by_sandy
            # raw_image.save_to_disk("_out/data_out/Depth/%08d", raw_image) 
            time.sleep(3)
